...
class Lexer:
    ...
    def process_macro(self) -> Token:
        ...
        while self.pos < len(self.stream) and (
            self.stream[self.pos] in self.macro_chars
...
class Compiler:
    ...
    glide_based = [
        ...
        "whilememregister",
        "movetoaddress",
        "sendboolmemdownaddress",
        "addmemprefix",
        "movememprefix",
        "movetonextmeminternal",
        "movetoprevmeminternal",
        "movetonextmem",
        "movetoprevmem",
        "endmemaccess",
        "loadbinx",
        ...
        while token.type != TokenType.EOF:
            if token.type == TokenType.Brainfuck:
                ...
                if macro not in self.macros:
                    raise NameError(
                        f"The bf-macro '{macro}' is not defined (pos: {lexer.pos - len(macro)})."
                    )
                ...
                if token.type != TokenType.Params:
                    raise SyntaxError(
                        f"Expected lexer type was {TokenType.Params.name} (got {token.type.name} instead)."
                    )
                params = token.value
                ...
                if macro not in self.placeholder_macros or params[0]:
                    for i in range(number):
                        ...
                        self.stack_trace_data[0][self.pc].append(
                            macro_id + f"_{i}" * (number != 1)
                        )
                        result += self.macros[macro](params)
                        if self.pc - 1 not in self.stack_trace_data[1]:
                            self.stack_trace_data[1][self.pc - 1] = []
                        self.stack_trace_data[1][self.pc - 1].insert(
                            0, macro_id + f"_{i}" * (number != 1)
                        )
                self.in_mem = before_mem
                number = 1
            else:
                raise SyntaxError(f"Unknown lexer type ({token.type.name}).")
            token = lexer.next_token()
        return result
    ...
    def param_to_number(self, param: str) -> int:
        ...
        if token.type != TokenType.Number:
            raise SyntaxError("This parameter expected a number.")
        return token.value
    ...
    def mac_divb(self, values: list[str]) -> str:
        return self.convert(
            "[2repeat(copyb(2)>)>lessb()ifel(5<upb(4)5>;3<+<copyb(4)<subb()6>downb(4)<)3<]>[-]>downb(1)3>downb(3)5<"
        )
    ...
    def mac_lessb(self, values: list[str]) -> str:
        return self.convert(
            "2repeat(copyb(1)>)diffb()ifel(3<[-<copyb(1)2>ifel(4<-4>;3<[-]4>+<)<]<[-]>[-]4>downb(4)<;4<[-]>[-]3>)2<"
        )
    ...
    def mac_ifel(self, values: list[str]) -> str:
        return self.convert(
            f"bool()[->>ifel_true({values[0]})<+<]->downb(0)<[+>>ifel_false({values[1]})<<]"
        )
    ...
    def mac_while(self, values: list[str]) -> str:
        return self.convert(
            f"while_bool({values[0]})bool()[-while_run({values[1]})while_bool({values[0]})bool()]"
        )
    ...
    def mac_bin8tobyte(self, values: list[str]) -> str:
        return self.convert(
            "[-8>128+8<]>[-7>64+7<]>[-6>32+6<]>[-5>16+5<]>[-4>8+4<]>[-3>4+3<]>[->>2+<<]>[->1+<]>downb(7)8<"
        )
    ...
    def mac_bytetobin8(self, values: list[str]) -> str:
        return self.convert(
            ">128+<divb()>>64+<divb()>>32+<divb()>>16+<divb()>>8+<divb()>>4+<divb()>>2+<divb()6<"
        )
    ...
    def mac_printb(self, values: list[str]) -> str:
        return self.convert(
            ">100+<divb()[48+.[-]]addb()>10+<divb()[48+.[-]]addb()48+.[-]"
        )
    ...
    def mac_printcleanintbinx(self, values: list[str]) -> str:
        ...
        if x == 1:
            ...
            while ten:
                if ten // 10:
                    data += f"copybinx({x};{x - 1}){2*x}>implant({x};{ten}){x}<divbinx({x}){x + 1}<copyb({2*x}){2*x + 1}>ifel(6<printdigit()6>;6<copybinx(4;5)printnzdigit()6>eqbinx(4)not()downb({2 + 2*x})){x}<implant({x};{ten}){x}<modbinx({x})"
                else:
                    data += f"{x - 4}>printdigit(){x - 4}<cleanbinx({x})<[-]"
                ten //= 10
        return self.convert(data)
    ...
    def mac_getintbinx(self, values: list[str]) -> str:
        x = self.param_to_number(values[0])
        return self.convert(
            f"{x}>while(,48-copyb(0)2>10+<lessb();<upb({6*x + 11})implant({x};10){x}<multbinx({x}){7*x + 12}>downb({6*x + 11}){6*x + 12}<{x - 4}repeat(upb(0)>)digitbin4(){2*x - 4}<addbinx({x}){x}>)<[-]{x}<"
        )
    ...
    def mac_andbinx(self, values: list[str]) -> str:
        x = self.param_to_number(values[0])
        return self.convert(
            f"{x}>{x}repeat(<upb({x - 1}){x}>>++<eqb()downb({x - 1}){x}<)"
        )
    ...
    def mac_addbinx(self, values: list[str]) -> str:
        ...
        x1 = x + 9
        return self.convert(
            f"{x}>{x}repeat(<upb({x+3}){x}>upb({6})>ifel(>ifel(>ifel(9<+{x}<+{x1}>;9<+9>)<;>ifel(9<+9>;{x1}<+{x1}>)<)<;>ifel(>ifel(9<+9>;{x1}<+{x1}>)<;>ifel({x1}<+{x1}>;)<)<){x + 1}<){x}>[-]{x}<"
        )
    ...
    def mac_subbinx(self, values: list[str]) -> str:
        ...
        x1 = x + 9
        return self.convert(
            f"{x}>{x}repeat(<upb({x+3}){x}>upb({6})>ifel(>ifel(>ifel(9<+{x}<+{x1}>;)<;>ifel(9<+9>;9<+{x}<+{x1}>)<)<;>ifel(>ifel(;{x1}<+{x1}>)<;>ifel(9<+{x}<+{x1}>;)<)<){x + 1}<){x}>[-]{x}<"
        )
    ...
    def mac_multbinx(self, values: list[str]) -> str:
        x = self.param_to_number(values[0])
        return self.convert(
            f"{x}>upbinx({x};{x + 1}){x}repeat(<ifel(>copybinx({2*x};{4*x - 1}){2*x}>addbinx({2*x}){2*x + 1}<;)3>lshiftbinx({2*x}){2+x}<rshiftbinx({x}){x}>)2>cleanbinx({2*x})@{2*x}>{x}lshiftbinx({2*x})downbinx({x};{x + 2 + 2*x - 1}){x + 2 + 2*x}<"
        )
    ...
    def mac_divbinx(self, values: list[str]) -> str:
        x = self.param_to_number(values[0])
        return self.convert(
            f"""upbinx({x};{3*x - 1}){x}>copybinx({x};{3*x - 1}){3*x}>eqbinx({x})ifel(kill();)implant({x};{x})while(
				while(
    ...
    def mac_modbinx(self, values: list[str]) -> str:
        x = self.param_to_number(values[0])
        return self.convert(
            f"2repeat(copybinx({x};{2*x - 1}){x}>)divbinx({x}){x}<multbinx({x}){x}<subbinx({x})"
        )
    ...
    def mac_lessbinx(self, values: list[str]) -> str:
        x = self.param_to_number(values[0])
        return self.convert(
            f"""copybinx({x};{2*x - 1}){x}>copybinx({x};{2*x - 1}){x}>diffbinx({x})upb(0)>ifel(
			  3<while(
    ...
    def mac_searchup255(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(">+[->+]-")
    ...
    def mac_sendboolup(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(
            "+3<copyb(0)>ifel(-searchup255()++>-searchdown255();-searchup255()+>-searchdown255())>>"
        )
    ...
    def mac_sendbooldown(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(
            "ifel(-searchdown255()+3<[-]+<-4>searchup255()+;-searchdown255()+3<[-]<-4>searchup255()+)"
        )
    ...
    def mac_sendbooldownaddress(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(
            "ifel(-searchdown255()++<->searchup255()+;-searchdown255()+<->searchup255()+)"
        )
    ...
    def mac_writeaddress(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(
            f"{self.memory_address_size}>-searchdown255()+{self.memory_address_size}>-searchup255()+{self.memory_address_size}repeat(<sendbooldownaddress())"
        )
    ...
    def mac_sendboolmemdownaddress(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(
            "+3<copyb(0)>ifel(-searchdown255()++>-searchup255();-searchdown255()+>-searchup255())>>"
        )
    ...
    def mac_addmemprefix(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(
            f"searchdown255(){self.memory_address_size + 1}>-{self.memory_address_size}repeat(movetonextmeminternal()sendboolmemdownaddress())searchdown255()+searchup255(){self.memory_address_size}repeat(movetoprevmeminternal())"
        )
    ...
    def mac_movememprefix(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(
            f"searchdown255()>-searchup255(){self.memory_address_size}repeat(movetonextmeminternal()sendboolmemdownaddress())searchdown255()+searchup255(){self.memory_address_size}repeat(movetoprevmeminternal())"
        )

    def mac_whilememregister(self, values: list[str]) -> str:
        return self.convert(
            f"while(>copybinx({self.memory_address_size};{self.memory_address_size - 1}){self.memory_address_size}>boolbinx({self.memory_address_size});{self.memory_address_size - 1}>+searchdown255()>subbinx({self.memory_address_size})searchup255(){values[0]}searchdown255())"
        )
    ...
    def mac_movetoaddress(self, values: list[str]) -> str:
        if not self.has_memory:
            raise MemoryError("Memory was never initiated.")
        return self.convert(
            f"-searchdown255()>{self.memory_address_size}>{self.memory_address_size}>9>3>-searchdown255()whilememregister(movetonextmeminternal())2searchup255()+"
        )
    ...
    def mac_loadbinx(self, values: list[str]) -> str:
        ...
        x = self.param_to_number(values[0])
        return self.convert(
            f"writeaddress()movetoaddress()-searchdown255(){self.memory_address_size + 1}movetonextmeminternal(){x}repeat(sendboolup()movetonextmeminternal())+searchup255()+{x}<"
        )
    ...
    def mac_savebinx(self, values: list[str]) -> str:
        ...
        x = self.param_to_number(values[0])
        return self.convert(
            f"{x}>writeaddress()movetoaddress()-searchdown255(){x + self.memory_address_size}movetonextmeminternal()searchup255()+{x}repeat(<sendbooldown())endmemaccess()"
        )
