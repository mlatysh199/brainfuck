...
class Token:
	...
	def __eq__(self, __value: object) -> bool:
		if type(__value) == Token: return (__value.type == self.type or __value.type.value == self.type.value) and __value.value == self.value
		elif isinstance(__value, Enum): return __value == self.type or __value.value == self.type.value
		return False
...
class Grammar(IntermediateForm):
	def __init__(self, base: IntermediateForm, catch_eof = True) -> None:
		if catch_eof: self.base = Concat([base, Terminal(TokenType.EOF)])
		else: self.base = base
...
def debug(func):
	def printer(self) -> list[ASTNode|Token]|None:
		...
		print_level[0] += 1
		result = func(self)
		if result == None: print(bcolors.FAIL, "|  "*print_level[0], "<!", self.__class__.__name__)
		else: print(bcolors.OKGREEN, "|  "*print_level[0], "<#", self.__class__.__name__)
		print_level[0] -= 1
		return result
	return printer
...
class Parser:
	...
	def __get_matcher_literal(self, data: Token|Enum) -> LiteralMatcher:
		...
		if not forget and type(data) == Token: forget = data.type in self.terminal_forget_set
		return LiteralMatcher(self.lexer, data, forget)
	
	def __get_matcher_terminal(self, terminal: Terminal) -> LiteralMatcher:
		if terminal not in self.matchers: self.matchers[terminal] = self.__get_matcher_literal(terminal.data)
		return self.matchers[terminal]

	def __get_matcher_concat(self, concat: Concat) -> ConcatMatcher:
		if concat not in self.matchers:
			matcher = None
			for i in reversed(concat.additions): matcher = ConcatMatcher(self.__get_matcher(i), matcher)
	...
	def __get_matcher_alter(self, alter: Alter) -> AlterMatcher:
		if alter not in self.matchers:
			matcher = None
			for i in reversed(alter.ors): matcher = AlterMatcher(self.__get_matcher(i), matcher)
	...
	def __get_matcher_count(self, count: Count) -> CountMatcher:
		if count not in self.matchers:
			self.matchers[count] = CountMatcher(self.__get_matcher(count.node), count.count_type)
	...
	def __get_matcher(self, IF: IntermediateForm) -> Matcher:
		t = type(IF)
		if t == Count: return self.__get_matcher_count(IF)
		elif t == RuleRef: return self.__get_matcher_rule(IF)
		elif t == Terminal: return self.__get_matcher_terminal(IF)
		elif t == Alter: return self.__get_matcher_alter(IF)
		elif t == Concat: return self.__get_matcher_concat(IF)
		elif t == Grammar: return self.__get_matcher_grammar(IF)
		elif t == IntermediateForm: return EmptyMatcher()
		else: raise SyntaxError("Wrong IF structure.") # TODO put correct error (SyntaxError should only happen during parsing????)
...
def show_IF(data: IntermediateForm, level = 0, showed = []) -> None:
	...
	t = type(data)
	showed.append(data)
	if t == Terminal: print(data.data)
	else:
		print("> ", end="")
		if t == Alter:
			print("Alter")
			for i in data.ors: show_IF(i, level + 1)
		elif t == Concat:
			print("Concat")
			for i in data.additions: show_IF(i, level + 1)
		elif t == RuleRef:
			print("Rule", data.rule_name)
