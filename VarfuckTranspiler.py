...
class Lexer(VP.BaseLexer):
	...
	def next_token(self) -> VP.Token:
		...
		if not self.do_cache or self.pos not in self.cache:
			if self.pos >= len(self.stream): self.cache[start_pos] = (self.pos, VP.Token(TokenType.EOF, None))
			else:
				...
				if self.pos not in self.cache:
					if char in self.word_chars:
						...
						self.pos += 1
						if char in self.bracket_chars: self.cache[start_pos] = (self.pos, VP.Token(TokenType.Bracket, char))
						elif char in self.brace_chars: self.cache[start_pos] = (self.pos, VP.Token(TokenType.Brace, char))
						elif char in self.break_chars: self.cache[start_pos] = (self.pos, VP.Token(TokenType.Breaker, None))
						elif char in self.separator_chars: self.cache[start_pos] = (self.pos, VP.Token(TokenType.Separator, None))
						elif char in self.parenthesis_chars: self.cache[start_pos] = (self.pos, VP.Token(TokenType.Parenthesis, char))
						else: raise KeyError(f"{char} is not a valid character.")
...
class ConstExpr:
	...
	def __str__(self) -> str:
		if len(self.refs): raise ValueError(f"ConstExpr hasn't been fully built ({self.data}).")
		if self.force_nnint:
			e = eval(self.data[0])
	...
	def __eq__(self, __value: object) -> bool:
		if type(__value) != ConstExpr: raise TypeError(f"Cannot compare ConstExpr with {type(__value)}.")
...
class BinXManager:
	...
	def goto(self, pos: str|None|ConstExpr) -> None:
		if pos != None and pos not in self.pos_table: raise NameError(f"No such variable defined as {pos}.")
		...
		if self.pos == None or ((not (pos == None)) and self.order_table[pos] < self.order_table[self.pos]):
			# go right
			self.add(a - b)
			self.add("<")
		else:
	...
	def pop_section(self) -> None:
		...
		for i in self.var_stack.pop(): self.del_var(i)
	
	def add_var(self, var: BinX, include = True) -> None:
		if var.name == None: raise NameError("Variable names must be complete.")
		if var.name in self.pos_table: raise NameError("Variable names must be non repeating.")
		self.size_table[var.name] = var.size
		self.pos_table[var.name] = self.size
		self.order_table[var.name] = self.vcount
		self.vcount += 1
		if include: self.var_stack[-1].append(var.name)
	...
	def do_call(self, mac: "MacroInvocation", param_data: list[str|None], params: list[ConstExpr], ret_data: list[str|None], ret: list[ConstExpr]) -> None:
		base = self.size
		if len(param_data) != len(params): raise TypeError(f"Expected {len(params)} parameters but got {len(param_data)}.")
		comp_param = []
		for i in range(len(param_data)):
			if param_data[i] != None:
				self.load_var(param_data[i])
				comp_param.append(self.size_table[param_data[i]])
			else: comp_param.append(params[i])
			self.size += params[i]
		mac.set_v_params(comp_param)
		self.size = base
		self.goto(None)
		self.add(mac)
		if len(ret_data) != len(ret): raise TypeError(f"Expected {len(ret)} return parameters but got {len(ret_data)}.")
		comp_ret = []
	...
	def end(self) -> None:
		if len(self.var_stack) != 1: raise OverflowError(f"Can't end macro while stack is not sufficiently empty.")
		# This should bring the stack pointer all the way down to 0
		rev = self.var_stack.pop()
		for i in range(len(rev) - 1, -1, -1):
			self.clear_var(rev[i])
			if i: self.goto(rev[i - 1])
			else: self.goto(None)
			self.del_var(rev[i])
...
class MacroInvocation:
	...
	def prepare(self, params: list[tuple[str, ConstExpr]]) -> None:
		...
		for i in range(len(v_params)):
			for j in range(len(params)): v_params[i] = v_params[i].replace(params[j][0], params[j][1])
		for i in range(len(ret)):
			for j in range(len(params)): ret[i] = ret[i].replace(params[j][0], params[j][1])
		t_v_params = self.test_params()
		t_ret = self.test_ret()
		for i in range(len(t_v_params)):
			for j in range(len(params)): t_v_params[i] = t_v_params[i].replace(params[j][0], params[j][1])
		for i in range(len(t_ret)):
			for j in range(len(params)): t_ret[i] = t_ret[i].replace(params[j][0], params[j][1])
		if t_v_params != v_params: raise TypeError(f"Expected expressions to be the same but got {v_params} and {t_v_params}.")
		if ret != t_ret: raise TypeError(f"Expected expressions to be the same but got {ret} and {t_ret}.")
...
class Processor:
	inbuilt_macros = {
		...
		"eqbinx" : Macro.build(
			"eqbinx",
			[("x", ConstExpr)],
			[BinX("binx", ConstExpr([ConstRef("x")])), BinX("biny", ConstExpr([ConstRef("x")]))],
			[ConstExpr(["1"])],
			[ConstExpr([ConstRef("x")])]
			),
	...
	def __param_struct(self, node: ASTNode) -> list[BinX]:
		data = []
		for i in range(0, len(node.data), 3): data.append(BinX(node.data[i + 1].value, self.__const_expr(node.data[i])))
		return data
			...
			for i in data.data:
				...
				if stmt.name == "const_def":
					...
					if invc[0].macro.name == "lessbinx":
						mac.data.goto(None)
						mac.data.add("@")
					mac.data.do_call(invc[0], invc[2], invc[0].test_params(), invc[1], invc[0].test_ret())
					if invc[0].macro.name == "lessbinx":
						mac.data.goto(None)
						mac.data.add("@")
					# mac.data.debug() # TODO
